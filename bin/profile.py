...
pc.defineParameter("node_type", "Hardware spec of nodes <br> Refer to manuals at <a href=\"http://docs.aptlab.net/hardware.html#%28part._apt-cluster%29\">APT</a> for more details.",
 portal.ParameterType.NODETYPE, "r320", legalValues=[("r320","APT r320"), ("c6220", "APT c6220")], advanced=False, groupId=None)
pc.defineParameter("num_nodes", "Number of hosts (not including OCS) to use.<br> Check cluster availability <a href=\"https://www.cloudlab.us/cluster-graphs.php\">here</a>.",
 portal.ParameterType.INTEGER, 8, legalValues=[], advanced=False, groupId=None)
pc.defineParameter("switch", "Preferred Mellanox switch", portal.ParameterType.STRING, "mellanox3", 
...
iface = nodes[-1].addInterface('if-switch', 
    pg.IPv4Address('10.2.100.100', '255.255.255.0'))
nodes[-1].hardware_type = params.node_type
nodes[-1].disk_image = SWITCH_DISK_IMAGE
nodes[-1].Desire(params.switch, 1.0)
nodes[-1].addService(pg.Install('https://github.com/mukerjee/etalon/archive/master.tar.gz','/local/'))
...
for i in range(1, params.num_nodes+1):
    ...
    nodes[i].addService(pg.Install('https://github.com/mukerjee/etalon/archive/master.tar.gz','/local/'))
...
instructions = """
Use this profile to instantiate a set of nodes on the APT cluster, all bound to the same mellanox switch, for use with Etalon.
"""
