...
void usage(char *name)
{
    printf("Execution utility for Mininet\n\n"
           ...
           "  -n tdf: run in new network and mount namespaces with specific TDF\n"
           "  -t tdf: set a new TDF using settimedilationfactor\n"
           "  -p: print ^A + pid\n"
...
int main(int argc, char *argv[])
{
    ...
    while ((c = getopt(argc, argv, "+cdn:t:pa:g:r:vh")) != -1) {
        switch(c) {
        case 'c':
        ...
        case 't':
            tdf = atoi(optarg);
            int ppid = getppid(); // maybe ppid, by which we ID host, should give as argument parameter
            /* try set tdf to be another value */
            if (settimedilationfactor(tdf, ppid) == -1) {
                perror("settimedilationfactor");
                printf("[process %d child of %d] settimedilationfactor fail\n", getpid(), getppid());
                return 1;
            }
            // printf("[process %d child of %d] settimedilationfactor success\n", getpid(), getppid());
            break;
        case 'p':
            /* print pid */
            printf("\001%d\n", getpid());
