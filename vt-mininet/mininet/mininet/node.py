...
class Node( object ):
    ...
    def setTDF(self, tdf):
        self.tdf = tdf
        cmd = "mnexec -p -t %s" % tdf
        out = self.cmd( cmd, printPid = True )
        # print "*** Change TDF to %s for %s (pid = %s) in mnexec %s" % ( tdf, self.name, self.pid, out )
    ...
    def startShell( self, mnopts=None ):
        ...
        if self.inNamespace:
            opts += 'n %s' % self.tdf
            if self.tdf != 1:
                print "[info] %s is about to set tdf %s in mnexec" % ( self.name, self.tdf )
                print
        ...
        # let's see node's pid
        # print
        # if self.name.startswith('h'):
        #     print "Host %s's pid: %s\n" % ( self.name, self.pid)
        # elif self.name.startswith('s'):
        #     print "Switch %s's pid: %s\n" % ( self.name, self.pid)
        # elif self.name.startswith('c'):
        #     print "Controller %s's pid: %s\n" % ( self.name, self.pid)
...
class CPULimitedHost( Host ):
    ...
    def setCPUFrac( self, f=-1, sched=None):
        ...
        # print '[cg] CPULimitedHost: (%s %d/%dus) ' % ( sched, quota, period )
