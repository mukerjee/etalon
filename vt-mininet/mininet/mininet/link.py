...
class TCIntf( Intf ):
    ...
    def bwCmds( self, bw=None, speedup=0, use_hfsc=False, use_tbf=False,
        ...
        # I will let mininet try leq 100Gbps bw 
        if bw and ( bw < 0 or bw > 100000 ):
            # error( 'Bandwidth', bw, 'is outside range 0..1000 Mbps\n' )
            error( 'Bandwidth', bw, 'is outside range 0..10000 Mbps\n')
            ...
            if use_hfsc:
                ...
                if latency_ms is None:
                    latency_ms = 15 * 8 / bw
                print "Use tbf with latency as", latency_ms, " ms"
                cmds += [ '%s qdisc add dev %s root handle 5:0 tbf ' +
...
class Link( object ):
    ...
    def __init__( self, node1, node2, port1=None, port2=None,
        ...
        if 'port' not in params1:
            self.port1 = node1.newPort()
            params1[ 'port' ] = self.port1
        if 'port' not in params2:
            self.port2 = node2.newPort()
            params2[ 'port' ] = self.port2
        if not intfName1:
