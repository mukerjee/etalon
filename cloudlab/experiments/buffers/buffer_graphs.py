...
experiment_names = [
    ...
    # ('all_pairs', 1505675739),
    # ('ring', 1505676207),
    # ('one_to_all', 1505676725),
    ('one_to_all', 1505700792),
    # ('all_to_one', 1505677277),
...
pattern_labels = ['One to All']
schedule_labels = ['Solstice', 'Resize'] #'No Circuit', 'Strobe', 'Resize']
...
for pattern, t in experiment_names:
    ...
    options.output_fn = 'graphs/%s-buffer_size_vs_latency90.pdf' % pattern
    options.x.label.xlabel = 'Buffer size (packets)'
    options.y.label.ylabel = 'Latency (us)'
    plot(x, y, options)
    ...
    options.output_fn = 'graphs/%s-buffer_size_vs_latency99.pdf' % pattern
    options.x.label.xlabel = 'Buffer size (packets)'
    options.y.label.ylabel = 'Latency (us)'
    plot(x, y, options)
    ...
    options.series_options = [DotMap(color='C%d' % i, marker='o', linewidth=1,
                                     yerr=yerr[i]) for i in range(len(x))]
    options.output_fn = 'graphs/%s-bufer_size_vs_up_util.pdf' % pattern
    options.x.label.xlabel = 'Buffer size (packets)'
    # options.y.label.ylabel = 'Rack throughput (Gbps)'
    options.y.label.ylabel = 'Avg. Cir. Up Link Util. (%)'
    # options.y.limits = [0, 10]
    ...
    options.series_options = [DotMap(color='C%d' % i, marker='o', linewidth=1,
                                     yerr=yerr[i]) for i in range(len(x))]
    options.output_fn = 'graphs/%s-bufer_size_vs_down_util.pdf' % pattern
    options.x.label.xlabel = 'Buffer size (packets)'
    # options.y.label.ylabel = 'Rack throughput (Gbps)'
    options.y.label.ylabel = 'Avg. Cir. Down Link Util. (%)'
    # options.y.limits = [0, 10]
