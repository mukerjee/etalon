...
plot(c(0, ts_max), c(0, 7.2), xaxt='n', type = 'n', bty='n', xlab = 'Time (us)' , yaxt= 'n', ylab = '', cex.lab= 1, cex.axis=1.2);
...
for (i in seq(1, trace_dim[1])) { # trace_dim[1]: rows
	if (trace_info[i, 3] == 4) { #PFC info
		...
		left_corner <- trace_info[i, 2] + 0.2
		right_corner <- trace_info[i, 2] + 0.8
...
par(xpd=TRUE);
# for (y in seq(0, 6)) { text(-200, y+0.25, 'Ack', cex=0.7) }
# for (y in seq(1, 7)) { text(-180, y, 'Data', cex=0.7) }
for (y in seq(0, 6)) { 
    host <- paste(6-y, ' -> 7', sep=''); 
    text(-180, y+0.5, host, cex=0.8);
}

week_start_times = seq(0,2) * 1500
text(week_end+60, 7.5, 'Apply', cex=0.8, pos=2)
text(week_end+30, -0.1, week_start_times, cex=0.8, pos=1)
text(week_end[1]-30, -0.5, 'time in trace = 1.7024702s', cex=0.7, pos=1);
...
legend_list <- c();
for (i in seq(0,6)) { 
	flow_name <- paste('host', 6-i, ' -> ', 7, sep='');
	legend_list <- c(legend_list, flow_name);
}
# print(legend_list)
legend(3.12, 9.8, rev(legend_list), pch=15, col = rev(my_col[1:7]), bty='n',
cex=1, y.intersp=1.2) 
# dev.off();
